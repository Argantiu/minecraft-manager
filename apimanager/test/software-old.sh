...
craftbukkit) SOFTAPI=https://hub.spigotmc.org/jenkins/job/BuildTools/lastSuccessfulBuild/artifact/target/BuildTools.jar ;;
...
if ! unzip -qq -t BuildTools-1.jar; then #if [ "$?" -ne 0 ]; then
 ...
 if [ "$?" -eq 1 ]; then 
  ...
  #cp ./Craftcraftbukkit/target/spigot-*.jar "$MTPATH"/mcsys/saves/jar/spigot-"$MAINVERSION".jar"$(date +%Y.%m.%d.%H.%M.%S)"
  #mv ./Craftcraftbukkit/target/spigot-*.jar "$MTPATH"/"$MCNAME".jar
  cd ../ || exit 1
  rm -r ./cache

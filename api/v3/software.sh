...
echo "Downloaded paper-$MCVERSION-$LATEST.jar is corrupt. No update." $MCDEBUG
else 
diff -q paper-"$MCVERSION"-"$LATEST".jar "$MTPATH"/"$MCNAME".jar >/dev/null 2>&1
 if [ "$?" -eq 1 ]; then cp paper-"$MCVERSION"-"$LATEST".jar paper-"$MCVERSION"-"$LATEST".jar."$(date +%Y.%m.%d.%H.%M.%S)" && mv paper-"$MAINVERSION"-"$LATEST".jar "$MTPATH"/"$MCNAME".jar
  /usr/bin/find "$MCPATH"/mcsys/saves/jar/* -type f -mtime +10 -delete 2>&1
  echo "paper-$MCVERSION-$LATEST has been updated" $MCDEBUG
  rm -f version.json; else 
  echo "No paper-$MCVERSION-$LATEST update neccessary" $MCDEBUG

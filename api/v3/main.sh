...
mcstart() {
    ...
    if screen -list | grep -q "$MCNAME"; then 
        echo -e "$(jq -r .mcstart.online ./libraries/mcsys/messages.json | sed "s:%s_name%:$MCNAME:g")" &&
        exit 1; 
    else 
        echo -e "$(jq -r .mcstart.start ./libraries/mcsys/messages.json | sed "s:%s_name%:$MCNAME:g")"; 
    fi
    if [ ! -f "$MCPATH"/"$MCNAME".jar ]; then 
        touch "$MCPATH"/"$MCNAME".jar; 
    fi
    if [ ! -f "$MCPATH"/libraries/mcsys/updater.sh ]; then 
        touch "$MCPATH"/libraries/mcsys/updater.sh;
    fi
    ...
    if [[ $(yq eval '.backup' mcsys.yml) == "true" ]] && [ -f "$MCNAME.jar" ]; then 
        echo -e "$(jq -r .mcstart.backup.create ./libraries/mcsys/messages.json)"
        mkdir -p "$MCPATH"/libraries/mcsys/backups && 
        cd "$MCPATH"/libraries/mcsys/backups && 
        usr/bin/find "$MCPATH"/libraries/mcsys/backups/* -type f -mtime +10 -delete 2>&1 #ls -1tr | head -n -10 | xargs -d '\n' rm -f --
        cd "$MCPATH" || exit 1
        tar -pzcf ./libraries/mcsys/backups/backup-"$MCNAME"-"$(date +%Y.%m.%d.%H.%M.%S)".tar.gz --exclude="$MCNAME.jar" --exclude="cache/*" --exclude="logs/*" --exclude="libraries/*" --exclude="paper.yml-README.txt" --exclude="screenlog.*" --exclude="versions/*" ./
        echo -e "$(jq -r .mcstart.backup.finish ./libraries/mcsys/messages.json)"; 
    fi
    if [[ $(yq eval '.proxy' mcsys.yml) == "true" ]] && [[ $MCSOFT == "bungeecord" ]] || [[ $MCSOFT == "waterfall" ]]; then 
...
mcstop() {
    if ! screen -list | grep -q "$MCNAME"; then 
        echo -e "$(jq -r .mcstop.offline ./libraries/mcsys/messages.json)" && exit 1; 
    fi
    ...
    if ! [[ $MCSOFTWARE == "bungeecord" ]] || [[ $MCSOFTWARE == "velocity" ]] || [[ $MCSOFTWARE == "waterfall" ]] && [[ $(yq eval '.count' mcsys.yml) == "true" ]]; then
        ...
        if grep -q error "on-i.txt"; then 
            echo -e "$(jq -r .counter.invalid ./libraries/mcsys/messages.json)"
        else 
            MCOTYPE=$(cat < on-i.txt | grep online | tr -d " " | cut -b 10)
        fi
    fi
    if ! [[ $MCOTYPE = "0" ]]; then 
        screen -Rd "$MCNAME" -X stuff "say $(jq -r .counter.stop + 10 + .counter.sec ./libraries/mcsys/messages.json) $(printf '\r')" && sleep 6s
...
mcrestart() { 
    if ! screen -list | grep -q "$MCNAME"; then 
        echo -e "$(jq -r .mcstop.offline + .mcstart.start ./libraries/mcsys/messages.json | sed "s:%s_name%:$MCNAME:g")" && 
        mcstart && 
        exit 0
    else
        mcstop &
...
mcdelete() { 
    ...
    if [[ $MCONFIRM == "ja" ]] || [[ $MCONFIRM == "yes" ]]; then
        ...
        cd ./libraries || exit 1
        rm -r ./mcsys && 
        echo -e "$(jq -r .tool.rm_ok ./libraries/mcsys/messages.json)" && 
        rm -- "$0"
    else 
        echo -e "$(jq -r .tool.rm_no ./libraries/mcsys/messages.json)"
    fi
    exit 0;
}

help() { 
    jq -r .tool.help ./libraries/mcsys/messages.json;
}

...
GEYSERAPI=https://download.geysermc.org/v2/projects/geyser/versions/latest/builds/latest/downloads/
...
case "$SOFTWARE" in
paper|purpur|spigot) 
if [[ $([ -f  ] && cat < "$MCPATH"/configs/paper-global.yml | grep velocity: | cut -d " " -f2) == "true" ]] || [[ $(cat < "$MCPATH"/spigot.yml | grep bungeecord: | cut -d " " -f2) == "true" ]]; then 
...
*) echo "Error!: Software doesn't support Bedrock Edition." ;;
esac
exit 0
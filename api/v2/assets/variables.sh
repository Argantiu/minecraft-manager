...
MANAGET2=$(cat < $MLANG | grep "manage.tool.remove:" | cut -d ':' -d '"' -f2 | $PLACEAT)
MANAGET3=$(cat < $MLANG | grep "manage.tool.remove.ok:" | cut -d ':' -d '"' -f2 | $PLACEAT)
MANAGET4=$(cat < $MLANG | grep "manage.tool.remove.no:" | cut -d ':' -d '"' -f2 | $PLACEAT)
MANAGET5=$(cat < $MLANG | grep "manage.tool.no.input:" | cut -d ':' -d '"' -f2 | $PLACEAT)
#start.sh
MSTART1=$(cat < $MLANG | grep "startsh.server.online:" | cut -d ':' -d '"' -f2 | $PLACEAT)
MSTART2=$(cat < $MLANG | grep "startsh.server.start:" | cut -d ':' -d '"' -f2 | $PLACEAT)
MSTART3=$(cat < $MLANG | grep "startsh.backup.create:" | cut -d ':' -d '"' -f2 | $PLACEAT)
MSTART4=$(cat < $MLANG | grep "startsh.backup.folder:" | cut -d ':' -d '"' -f2 | $PLACEAT)
MSTART5=$(cat < $MLANG | grep "startsh.bedrock.update:" | cut -d ':' -d '"' -f2 | $PLACEAT)
#stop.sh
SHSTOP1=$(cat < $MLANG | grep "stopsh.server.offline:" | cut -d ':' -d '"' -f2 | $PLACEAT)
SHSTOP2=$(cat < $MLANG | grep "stopsh.server.stop.info:" | cut -d ':' -d '"' -f2 | $PLACEAT)
SHSTOP3=$(cat < $MLANG | grep "stopsh.counter.invalid:" | cut -d ':' -d '"' -f2 | $PLACEAT)
SHSTOP4=$(cat < $MLANG | grep "stopsh.counter.stop.10:" | cut -d ':' -d '"' -f2 | $PLACEAT)
SHSTOP5=$(cat < $MLANG | grep "stopsh.counter.stop.4:" | cut -d ':' -d '"' -f2 | $PLACEAT)
SHSTOP6=$(cat < $MLANG | grep "stopsh.counter.stop.3:" | cut -d ':' -d '"' -f2 | $PLACEAT)
